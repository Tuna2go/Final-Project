...
#include <iostream>
using namespace std;
/*int main(){ //for testing purpose, may be delete or change later
...
int main(int argc, char* argv[]) {
  ...
  sp-=a;
  if(argc>3) {
    cout<<"Pick one student"<<endl;
    for(int a=1, i=1;a<argc;a+=2,i++){
        cout<<i<<". "<<argv[a]<<endl;
    }
    cin>>pickstudent;
    }
  ...
  while(menu!=4){
    sp[pickstudent-1].too_depressed(); // This line will cause the program to end if anxiety >= 10
    print_menu();
    cin>>menu;
    if(menu>4){
    ...
    switch (menu){
      ...
      case 3:
        if(canbepick1 == false && canbepick2 == false && canbepick3 == false){
          sp[pickstudent-1].newmenuscore();
        }
        else cout<<"Please finish all the quizzes before checking the result"<<endl;
      break;
      default: break;
    }
  }
