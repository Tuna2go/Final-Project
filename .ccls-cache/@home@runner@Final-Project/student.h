...
void Student::newmenuscore(){
  LL l;
  l.insert_node("Quiz 1", pointqone,5);
  l.insert_node("Quiz 2", pointqtwo,5);
  l.insert_node("Quiz 3", pointqthree,8);
  presstomove();
  l.print();
  char move;
  int exit=0;
  while(exit != 1){
    move = getch();
    cout << "\033[2J\033[0;0H"; //clear the screen
    presstomove();
    if(move == 'e'){
      l.move_right();
    }
...
void Student::showresult(){ //old result display
...
int Student::quiz_three(){
  ...
  char option[][1000] = {"A.Spongebob", "B.Squidward", "C.Patrick",
                        "A.Misses Puff", "B.Sheldon", "C.Gary",
  ...
  cout<<"Quiz 3 score:"<<pointqthree<<"/8"<<endl;
  cout<<"Current score:"<<score<<"/100"<<endl;
